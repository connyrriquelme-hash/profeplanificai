...
export async function onRequestGet(context: EventContext<Env>): Promise<Response> {
  try {
    ...
    console.error('[non-teaching-blocks] GET error:', err);
    return json({ error: 'Error al obtener los bloques.' }, 500);
  }
}
...
export async function onRequestPost(context: EventContext<Env>): Promise<Response> {
  try {
    ...
    if (!title || !blockDate) {
      return json({ ok: false, error: 'Titulo y fecha son requeridos' }, { status: 400 });
    }
    ...
    console.error('[non-teaching-blocks] POST error:', err);
    return json({ error: 'Error al crear el bloque.' }, 500);
  }
}
...
export async function onRequestPatch(context: EventContext<Env>): Promise<Response> {
  try {
    ...
    console.error('[non-teaching-blocks] PATCH error:', err);
    return json({ error: 'Error al actualizar el bloque.' }, 500);
  }
}
...
export async function onRequestDelete(context: EventContext<Env>): Promise<Response> {
  try {
    ...
    console.error('[non-teaching-blocks] DELETE error:', err);
    return json({ error: 'Error al eliminar el bloque.' }, 500);
  }
}
