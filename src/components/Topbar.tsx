...
export function Topbar({ title, onNavigate }: TopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-[var(--border)] px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-3 sm:gap-4 bg-white/80 backdrop-blur-xl" style={{ minHeight: '80px' }}>
      ...
      <div className="hidden lg:flex items-center gap-2.5 px-3 py-2 rounded-xl border border-[var(--border)] bg-[var(--primary-tint)]/30 cursor-pointer hover:bg-[var(--primary-tint)] transition-colors">
        ...
        <div className="hidden xl:block">
          ...
          <p className="text-[12px] font-semibold text-[var(--ink)] leading-tight truncate max-w-[140px]">Liceo Nacional</p>
        </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 ml-auto">
        <button className="p-2.5 rounded-xl text-[var(--muted)] hover:text-[var(--ink)] hover:bg-[var(--bg2)] transition-all min-w-[40px] min-h-[40px] flex items-center justify-center" aria-label="Ayuda">
          <HelpCircle size={18} strokeWidth={2} />
        </button>

        <button className="p-2.5 rounded-xl text-[var(--muted)] hover:text-[var(--ink)] hover:bg-[var(--bg2)] transition-all relative min-w-[40px] min-h-[40px] flex items-center justify-center" aria-label="Notificaciones">
        ...
        <div className="hidden sm:flex items-center gap-2.5 pl-3 ml-1 border-l border-[var(--border)]">
          <div className="w-9 h-9 rounded-full bg-[var(--primary)] flex items-center justify-center text-white text-xs font-bold shadow-sm">
            {initials}
          </div>
          <div className="hidden md:block">
            <p className="text-[13px] font-semibold text-[var(--ink)] leading-tight">{user?.nombre || 'Profe'}</p>
