...
type SubjectCategory = 'ciencias' | 'lenguaje' | 'matematica' | 'historia' | 'artes' | 'musica' | 'tecnologia' | 'formacion_ciudadana' | 'orientacion' | 'ingles' | 'educacion_fisica' | 'filosofia' | 'biologia' | 'fisica' | 'quimica' | 'ciencias_ciudadania' | 'parvularia' | 'general';
...
function detectSubjectCategory(subject: string): SubjectCategory {
  ...
  if (s.includes('educación parvularia') || s.includes('educacion parvularia') || s.includes('parvularia') || s.includes('pre-kinder') || s.includes('prekinder') || s.includes('kinder') || s.includes('identidad y autonomía') || s.includes('identidad y autonomia') || s.includes('convivencia y ciudadanía') || s.includes('convivencia y ciudadania') || s.includes('corporalidad y movimiento') || s.includes('lenguaje verbal') || s.includes('pensamiento matemático') || s.includes('pensamiento matematico') || s.includes('exploración del entorno') || s.includes('exploracion del entorno')) return 'parvularia';
...
function buildCriteriaBank(category: SubjectCategory, keywords: string[], oaText: string): CriteriaTemplate[] {
  ...
  switch (category) {
    ...
    case 'musica': return buildMusicaCriteria(keywords, oaText, mainConcept);
    case 'educacion_fisica': return buildEducacionFisicaCriteria(keywords, oaText, mainConcept);
    case 'filosofia': return buildFilosofiaCriteria(keywords, oaText, mainConcept);
    case 'biologia': return buildBiologiaCriteria(keywords, oaText, mainConcept);
    case 'fisica': return buildFisicaCriteria(keywords, oaText, mainConcept);
    case 'quimica': return buildQuimicaCriteria(keywords, oaText, mainConcept);
    case 'ciencias_ciudadania': return buildCiudadaniaCriteria(keywords, oaText, mainConcept);
    case 'parvularia': return buildParvulariaCriteria(keywords, oaText, mainConcept);
...
function buildMusicaCriteria(_keywords: string[], _oaText: string, _mainConcept: string): CriteriaTemplate[] {
  return [
    { name: 'Escucha activa', description: 'Escucha con atención y reconoce elementos sonoros', descriptors: { avanzado: { text: 'Identifica ritmo, melodía, timbre y dinámica con precisión', evidence: 'Análisis musical detallado', feedback: 'Tu escucha es muy atenta y precisa.' }, adecuado: { text: 'Reconoce al menos 3 elementos del sonido', evidence: 'Identificación de elementos musicales', feedback: 'Bien identificas los sonidos. Profundiza en el análisis.' }, en_desarrollo: { text: 'Reconoce algunos elementos con apoyo', evidence: 'Reconocimiento con guía', feedback: 'Estás desarrollando tu oído musical.' }, inicial: { text: 'Necesita apoyo significativo para escuchar activamente', evidence: 'Escucha con asistencia', feedback: 'Vamos a escuchar juntos con atención.' } } },
    { name: 'Interpretación rítmica y melódica', description: 'Interpreta ritmos y melodías de forma personal', descriptors: { avanzado: { text: 'Interpreta con precisión rítmica y melódica, añadiendo expresión personal', evidence: 'Interpretación completa y expresiva', feedback: 'Tu interpretación es expresiva y precisa.' }, adecuado: { text: 'Interpreta la pieza con ritmo y melodía correctos', evidence: 'Interpretación correcta', feedback: 'Buena interpretación. Agrega más expresión.' }, en_desarrollo: { text: 'Interpreta con apoyo del docente o compañeros', evidence: 'Interpretación con guía', feedback: 'Estás mejorando tu interpretación. Sigue practicando.' }, inicial: { text: 'Necesita modelado directo para interpretar', evidence: 'Interpretación con asistencia', feedback: 'Vamos a interpretar juntos.' } } },
    { name: 'Creación sonora', description: 'Crea sonidos, ritmos o pequeñas composiciones', descriptors: { avanzado: { text: 'Crea composiciones originales con intención y estructura', evidence: 'Composición original documentada', feedback: 'Tu creación es original y bien estructurada.' }, adecuado: { text: 'Crea ritmos o sonidos con variaciones', evidence: 'Creación con variaciones', feedback: 'Buena creación. Prueba nuevas combinaciones.' }, en_desarrollo: { text: 'Crea con apoyo y usando modelos', evidence: 'Creación con modelado', feedback: 'Estás creando con más confianza.' }, inicial: { text: 'Requiere guía para crear sonidos', evidence: 'Creación con asistencia', feedback: 'Vamos a crear juntos algo nuevo.' } } },
    { name: 'Expresión y reflexión musical', description: 'Expresa emociones a través de la música y reflexiona sobre ella', descriptors: { avanzado: { text: 'Expresa emociones con intención y reflexiona críticamente sobre la música', evidence: 'Reflexión profunda y expresión', feedback: 'Tu expresión y reflexión musical son sobresalientes.' }, adecuado: { text: 'Expresa emociones y menciona por qué le gusta la música', evidence: 'Expresión con reflexión', feedback: 'Bien expresas lo que sientes. Profundiza tu reflexión.' }, en_desarrollo: { text: 'Expresa emociones con apoyo y participa en reflexiones', evidence: 'Expresión con guía', feedback: 'Estás aprendiendo a expresarte musicalmente.' }, inicial: { text: 'Necesita apoyo para expresar emociones a través de la música', evidence: 'Expresión con asistencia', feedback: 'Vamos a expresar juntos lo que sentimos.' } } },
  ];
}

function buildEducacionFisicaCriteria(_keywords: string[], _oaText: string, _mainConcept: string): CriteriaTemplate[] {
  return [
    { name: 'Ejecución motriz', description: 'Ejecuta habilidades motrices con coordinación y control', descriptors: { avanzado: { text: 'Ejecuta habilidades con coordinación, control y precisión, adaptándose a diferentes contextos', evidence: 'Demostración de habilidad motriz', feedback: 'Tu ejecución motriz es excelente y fluida.' }, adecuado: { text: 'Ejecuta la habilidad con control básico', evidence: 'Ejecución correcta', feedback: 'Buena ejecución. Sigue perfeccionando la técnica.' }, en_desarrollo: { text: 'Ejecuta con apoyo y modelado del docente', evidence: 'Ejecución con guía', feedback: 'Estás mejorando tu coordinación. Practica más.' }, inicial: { text: 'Necesita modelado directo para ejecutar', evidence: 'Ejecución con asistencia', feedback: 'Vamos a practicar juntos el movimiento.' } } },
    { name: 'Respeto de reglas y normas', description: 'Participa respetando las reglas del juego o actividad', descriptors: { avanzado: { text: 'Participa respetando todas las reglas y ayuda a otros a cumplirlas', evidence: 'Participación respetuosa y colaborativa', feedback: 'Eres un ejemplo de respeto por las reglas.' }, adecuado: { text: 'Cumple la mayoría de las reglas del juego', evidence: 'Participación con cumplimiento', feedback: 'Bien respetas las reglas. Recuerda todas.' }, en_desarrollo: { text: 'Cumple reglas con recordatorio del docente', evidence: 'Participación con recordatorio', feedback: 'Estás aprendiendo a jugar con reglas.' }, inicial: { text: 'Necesita apoyo constante para respetar reglas', evidence: 'Participación con apoyo', feedback: 'Vamos a recordar las reglas juntos.' } } },
    { name: 'Autocuidado y seguridad', description: 'Aplica hábitos de autocuidado durante la actividad física', descriptors: { avanzado: { text: 'Aplica autocuidado de forma autónoma: calienta, se hidrata, cuida su cuerpo y el de otros', evidence: 'Registro de hábitos de autocuidado', feedback: 'Tus hábitos de autocuidado son ejemplares.' }, adecuado: { text: 'Realiza calentamiento y cuida su seguridad', evidence: 'Participación con autocuidado', feedback: 'Bien cuidas tu seguridad. Sigue así.' }, en_desarrollo: { text: 'Aplica autocuidado con recordatorio', evidence: 'Autocuidado con guía', feedback: 'Estás aprendiendo a cuidarte durante la actividad.' }, inicial: { text: 'Necesita recordatorios constantes de seguridad', evidence: 'Autocuidado con apoyo', feedback: 'Vamos a cuidarnos juntos.' } } },
    { name: 'Trabajo colaborativo', description: 'Participa en actividades grupales de forma respetuosa', descriptors: { avanzado: { text: 'Colabora activamente, apoya a compañeros y contribuye al logro del equipo', evidence: 'Participación colaborativa destacada', feedback: 'Tu trabajo en equipo es excelente.' }, adecuado: { text: 'Participa en grupo y respeta los turnos', evidence: 'Colaboración básica', feedback: 'Bien trabajas en equipo. Sigue colaborando.' }, en_desarrollo: { text: 'Participa con apoyo del docente para colaborar', evidence: 'Colaboración con guía', feedback: 'Estás aprendiendo a trabajar en equipo.' }, inicial: { text: 'Necesita apoyo para participar en grupo', evidence: 'Colaboración con asistencia', feedback: 'Vamos a participar juntos en el grupo.' } } },
  ];
}

function buildFilosofiaCriteria(_keywords: string[], _oaText: string, _mainConcept: string): CriteriaTemplate[] {
  return [
    { name: 'Formulación de preguntas', description: 'Formula preguntas filosóficas relevantes y profundas', descriptors: { avanzado: { text: 'Formula preguntas abiertas, profundas que generan reflexión múltiple', evidence: 'Preguntas filosóficas formuladas', feedback: 'Tus preguntas generan debate profundo.' }, adecuado: { text: 'Formula al menos 2 preguntas relevantes al tema', evidence: 'Preguntas relevantes', feedback: 'Buenas preguntas. Profundiza más.' }, en_desarrollo: { text: 'Formula preguntas con apoyo del docente', evidence: 'Preguntas con guía', feedback: 'Estás aprendiendo a formular preguntas filosóficas.' }, inicial: { text: 'Necesita modelado para formular preguntas', evidence: 'Preguntas con asistencia', feedback: 'Vamos a formular juntos preguntas profundas.' } } },
    { name: 'Argumentación', description: 'Argumenta sus posturas con razones y ejemplos', descriptors: { avanzado: { text: 'Argumenta con razones sólidas, ejemplos claros y anticipa objeciones', evidence: 'Argumentación completa y fundamentada', feedback: 'Tu argumentación es sólida y clara.' }, adecuado: { text: 'Argumenta con al menos 2 razones y un ejemplo', evidence: 'Argumentación con razones', feedback: 'Bien argumentas. Agrega más ejemplos.' }, en_desarrollo: { text: 'Argumenta con apoyo y razones simples', evidence: 'Argumentación con guía', feedback: 'Estás desarrollando tu capacidad argumentativa.' }, inicial: { text: 'Requiere guía para argumentar', evidence: 'Argumentación con asistencia', feedback: 'Vamos a argumentar juntos con razones.' } } },
    { name: 'Análisis de perspectivas', description: 'Analiza diferentes posturas sobre una cuestión filosófica', descriptors: { avanzado: { text: 'Analiza múltiples perspectivas con profundidad y las relaciona entre sí', evidence: 'Análisis de perspectivas múltiples', feedback: 'Tu capacidad de análisis es sobresaliente.' }, adecuado: { text: 'Identifica al menos 2 perspectivas diferentes', evidence: 'Identificación de perspectivas', feedback: 'Bien identificas las perspectivas. Relaciona más.' }, en_desarrollo: { text: 'Reconoce que existen diferentes posturas con apoyo', evidence: 'Reconocimiento con guía', feedback: 'Estás aprendiendo a ver diferentes posturas.' }, inicial: { text: 'Necesita apoyo para reconocer perspectivas', evidence: 'Reconocimiento con asistencia', feedback: 'Vamos a explorar juntos las diferentes posturas.' } } },
    { name: 'Uso de ejemplos y diálogo respetuoso', description: 'Usa ejemplos para fundamentar y dialoga con respeto', descriptors: { avanzado: { text: 'Usa ejemplos variados y relevantes, y mantiene diálogo respetuoso y productivo', evidence: 'Uso de ejemplos y diálogo ejemplar', feedback: 'Tu diálogo y uso de ejemplos son excelentes.' }, adecuado: { text: 'Usa al menos 1 ejemplo y respeta opiniones distintas', evidence: 'Uso de ejemplo y respeto', feedback: 'Bien usas ejemplos. Sigue dialogando con respeto.' }, en_desarrollo: { text: 'Usa ejemplos con apoyo y escucha opiniones', evidence: 'Uso de ejemplos con guía', feedback: 'Estás mejorando en el diálogo filosófico.' }, inicial: { text: 'Necesita modelado para usar ejemplos y dialogar', evidence: 'Diálogo con asistencia', feedback: 'Vamos a dialogar juntos con respeto.' } } },
  ];
}

function buildBiologiaCriteria(_keywords: string[], _oaText: string, _mainConcept: string): CriteriaTemplate[] {
  return [
    { name: 'Observación de seres vivos', description: 'Observa y describe características de organismos', descriptors: { avanzado: { text: 'Observa con precisión y describe características morfológicas, fisiológicas y conductuales', evidence: 'Registro de observación detallado', feedback: 'Tu observación es muy precisa y completa.' }, adecuado: { text: 'Identifica al menos 3 características del organismo', evidence: 'Identificación de características', feedback: 'Bien observas. Describe más detalles.' }, en_desarrollo: { text: 'Identifica características básicas con apoyo', evidence: 'Observación con guía', feedback: 'Estás mejorando tu capacidad de observación.' }, inicial: { text: 'Necesita apoyo para observar y describir', evidence: 'Observación con asistencia', feedback: 'Vamos a observar juntos con atención.' } } },
    { name: 'Clasificación biológica', description: 'Clasifica organismos según criterios científicos', descriptors: { avanzado: { text: 'Clasifica usando criterios taxonómicos y explica las razones de la clasificación', evidence: 'Clasificación con justificación', feedback: 'Tu clasificación es precisa y bien fundamentada.' }, adecuado: { text: 'Clasifica al menos 2 grupos de organismos', evidence: 'Clasificación básica', feedback: 'Bien clasificas. Fundamenta tus criterios.' }, en_desarrollo: { text: 'Clasifica con apoyo y criterios simples', evidence: 'Clasificación con guía', feedback: 'Estás aprendiendo a clasificar organismos.' }, inicial: { text: 'Requiere guía para clasificar', evidence: 'Clasificación con asistencia', feedback: 'Vamos a clasificar juntos.' } } },
    { name: 'Explicación de procesos biológicos', description: 'Explica procesos como ciclo de vida, alimentación, ecología', descriptors: { avanzado: { text: 'Explica procesos biológicos con vocabulario científico y conexiones ecológicas', evidence: 'Explicación científica completa', feedback: 'Tu explicación es científica y clara.' }, adecuado: { text: 'Explica el proceso con vocabulario adecuado', evidence: 'Explicación correcta', feedback: 'Bien explicas el proceso. Usa más vocabulario científico.' }, en_desarrollo: { text: 'Explica con apoyo y vocabulario simple', evidence: 'Explicación con guía', feedback: 'Estás aprendiendo a explicar procesos biológicos.' }, inicial: { text: 'Necesita modelado para explicar', evidence: 'Explicación con asistencia', feedback: 'Vamos a explicar juntos el proceso.' } } },
    { name: 'Conexión con el entorno', description: 'Relaciona el contenido biológico con su entorno y realidad', descriptors: { avanzado: { text: 'Relaciona el contenido con situaciones reales y propone acciones de conservación', evidence: 'Conexión con realidad y propuesta', feedback: 'Tu conexión con el entorno es profunda.' }, adecuado: { text: 'Menciona al menos 1 conexión con su entorno', evidence: 'Conexión básica', feedback: 'Bien conectas. Piensa en más ejemplos reales.' }, en_desarrollo: { text: 'Reconoce conexiones con apoyo', evidence: 'Conexión con guía', feedback: 'Estás aprendiendo a relacionar con tu entorno.' }, inicial: { text: 'Necesita apoyo para conectar con la realidad', evidence: 'Conexión con asistencia', feedback: 'Vamos a pensar en cómo se relaciona con tu vida.' } } },
  ];
}

function buildFisicaCriteria(_keywords: string[], _oaText: string, _mainConcept: string): CriteriaTemplate[] {
  return [
    { name: 'Comprensión de leyes físicas', description: 'Comprende y aplica leyes y principios de la física', descriptors: { avanzado: { text: 'Comprende leyes físicas con precisión y las aplica a situaciones nuevas', evidence: 'Aplicación de leyes a problemas', feedback: 'Tu comprensión de las leyes físicas es excelente.' }, adecuado: { text: 'Comprende al menos 2 leyes y sus aplicaciones', evidence: 'Comprensión de leyes', feedback: 'Bien comprendes las leyes. Aplica más.' }, en_desarrollo: { text: 'Reconoce principios físicos con apoyo', evidence: 'Comprensión con guía', feedback: 'Estás construyendo tu comprensión de la física.' }, inicial: { text: 'Necesita modelado para comprender leyes', evidence: 'Comprensión con asistencia', feedback: 'Vamos a explorar juntos las leyes físicas.' } } },
    { name: 'Diseño experimental', description: 'Diseña experimentos para verificar hipótesis', descriptors: { avanzado: { text: 'Diseña experimentos con control de variables, mediciones precisas y análisis de error', evidence: 'Diseño experimental completo', feedback: 'Tu diseño experimental es riguroso.' }, adecuado: { text: 'Diseña un experimento básico con hipótesis', evidence: 'Diseño con hipótesis', feedback: 'Bien diseñaste. Controla más variables.' }, en_desarrollo: { text: 'Participa en diseño experimental con guía', evidence: 'Diseño con apoyo', feedback: 'Estás aprendiendo a diseñar experimentos.' }, inicial: { text: 'Requiere asistencia para diseñar experimentos', evidence: 'Diseño con asistencia', feedback: 'Vamos a diseñar juntos el experimento.' } } },
    { name: 'Análisis de datos', description: 'Analiza resultados y extrae conclusiones', descriptors: { avanzado: { text: 'Analiza datos con tablas, gráficos y estadísticas simples, extrayendo conclusiones fundamentadas', evidence: 'Análisis completo con gráficos', feedback: 'Tu análisis de datos es preciso y claro.' }, adecuado: { text: 'Organiza datos y saca conclusiones básicas', evidence: 'Análisis básico', feedback: 'Bien analizas. Usa más herramientas gráficas.' }, en_desarrollo: { text: 'Organiza datos con apoyo del docente', evidence: 'Análisis con guía', feedback: 'Estás aprendiendo a analizar datos.' }, inicial: { text: 'Necesita apoyo para organizar datos', evidence: 'Análisis con asistencia', feedback: 'Vamos a organizar juntos los datos.' } } },
    { name: 'Explicación de fenómenos', description: 'Explica fenómenos físicos usando principios científicos', descriptors: { avanzado: { text: 'Explica fenómenos con vocabulario preciso, diagramas y modelos conceptuales', evidence: 'Explicación científica completa', feedback: 'Tu explicación de fenómenos es sobresaliente.' }, adecuado: { text: 'Explica el fenómeno con vocabulario adecuado', evidence: 'Explicación correcta', feedback: 'Bien explicas. Usa más vocabulario técnico.' }, en_desarrollo: { text: 'Explica con apoyo y vocabulario simple', evidence: 'Explicación con guía', feedback: 'Estás aprendiendo a explicar fenómenos físicos.' }, inicial: { text: 'Necesita modelado para explicar fenómenos', evidence: 'Explicación con asistencia', feedback: 'Vamos a explicar juntos el fenómeno.' } } },
  ];
}

function buildQuimicaCriteria(_keywords: string[], _oaText: string, _mainConcept: string): CriteriaTemplate[] {
  return [
    { name: 'Comprensión de sustancias', description: 'Comprende propiedades y clasificación de sustancias', descriptors: { avanzado: { text: 'Clasifica sustancias con criterios químicos y explica sus propiedades a nivel molecular', evidence: 'Clasificación con justificación molecular', feedback: 'Tu comprensión de sustancias es profunda.' }, adecuado: { text: 'Identifica propiedades físicas y químicas básicas', evidence: 'Identificación de propiedades', feedback: 'Bien identificas las propiedades. Profundiza más.' }, en_desarrollo: { text: 'Reconoce algunas propiedades con apoyo', evidence: 'Reconocimiento con guía', feedback: 'Estás construyendo tu comprensión química.' }, inicial: { text: 'Necesita apoyo para identificar sustancias', evidence: 'Reconocimiento con asistencia', feedback: 'Vamos a explorar juntos las sustancias.' } } },
    { name: 'Diseño experimental químico', description: 'Diseña experimentos seguros para observar reacciones', descriptors: { avanzado: { text: 'Diseña experimentos con protocolo de seguridad, mediciones y control de variables', evidence: 'Diseño experimental seguro y riguroso', feedback: 'Tu diseño experimental es seguro y preciso.' }, adecuado: { text: 'Diseña un experimento básico con supervisión', evidence: 'Diseño con supervisión', feedback: 'Bien diseñaste. Recuerda siempre la seguridad.' }, en_desarrollo: { text: 'Participa en experimentos con guía del docente', evidence: 'Participación con apoyo', feedback: 'Estás aprendiendo a experimentar con seguridad.' }, inicial: { text: 'Requiere asistencia directa para experimentar', evidence: 'Experimentación con asistencia', feedback: 'Vamos a experimentar juntos con cuidado.' } } },
    { name: 'Vocabulario químico', description: 'Usa vocabulario químico preciso para comunicar', descriptors: { avanzado: { text: 'Usa vocabulario químico preciso y lo aplica en explicaciones y escritos', evidence: 'Uso de vocabulario técnico', feedback: 'Tu vocabulario químico es preciso.' }, adecuado: { text: 'Usa al menos 5 términos químicos correctamente', evidence: 'Uso de vocabulario básico', feedback: 'Bien usas el vocabulario. Sigue aprendiendo más términos.' }, en_desarrollo: { text: 'Usa vocabulario con apoyo y modelado', evidence: 'Uso de vocabulario con guía', feedback: 'Estás aprendiendo el vocabulario químico.' }, inicial: { text: 'Necesita apoyo para usar vocabulario químico', evidence: 'Vocabulario con asistencia', feedback: 'Vamos a aprender juntos los términos químicos.' } } },
    { name: 'Conexión con la vida cotidiana', description: 'Relaciona la química con situaciones de la vida diaria', descriptors: { avanzado: { text: 'Relaciona conceptos químicos con problemas reales y propone soluciones', evidence: 'Conexión con realidad y propuesta', feedback: 'Tu conexión con la vida real es excelente.' }, adecuado: { text: 'Menciona al menos 1 aplicación en la vida diaria', evidence: 'Conexión básica', feedback: 'Bien conectas. Piensa en más aplicaciones.' }, en_desarrollo: { text: 'Reconoce aplicaciones con apoyo', evidence: 'Conexión con guía', feedback: 'Estás aprendiendo a relacionar con la vida real.' }, inicial: { text: 'Necesita apoyo para conectar con la realidad', evidence: 'Conexión con asistencia', feedback: 'Vamos a pensar en cómo la química está en tu vida.' } } },
  ];
}

function buildCiudadaniaCriteria(_keywords: string[], _oaText: string, _mainConcept: string): CriteriaTemplate[] {
  return [
    { name: 'Comprensión de problemas sociales', description: 'Comprende problemas sociales con evidencia científica', descriptors: { avanzado: { text: 'Analiza problemas sociales con evidencia científica multidimensional', evidence: 'Análisis con evidencia', feedback: 'Tu comprensión de problemas es profunda.' }, adecuado: { text: 'Identifica al menos 2 dimensiones de un problema social', evidence: 'Identificación de dimensiones', feedback: 'Bien analizas. Considera más perspectivas.' }, en_desarrollo: { text: 'Reconoce problemas con apoyo del docente', evidence: 'Reconocimiento con guía', feedback: 'Estás aprendiendo a analizar problemas sociales.' }, inicial: { text: 'Necesita apoyo para comprender problemas', evidence: 'Reconocimiento con asistencia', feedback: 'Vamos a explorar juntos este problema.' } } },
    { name: 'Uso de evidencia científica', description: 'Usa datos y evidencia para fundamentar argumentos', descriptors: { avanzado: { text: 'Usa evidencia científica variada y la integra en argumentos sólidos', evidence: 'Uso de evidencia múltiple', feedback: 'Tu uso de evidencia es riguroso.' }, adecuado: { text: 'Usa al menos 1 fuente de evidencia para argumentar', evidence: 'Uso de evidencia básica', feedback: 'Bien usas la evidencia. Busca más fuentes.' }, en_desarrollo: { text: 'Usa evidencia con apoyo del docente', evidence: 'Uso de evidencia con guía', feedback: 'Estás aprendiendo a usar evidencia científica.' }, inicial: { text: 'Necesita modelado para usar evidencia', evidence: 'Uso de evidencia con asistencia', feedback: 'Vamos a buscar juntos la evidencia.' } } },
    { name: 'Análisis de impacto', description: 'Analiza el impacto de decisiones en la sociedad y medio ambiente', descriptors: { avanzado: { text: 'Analiza impactos a corto y largo plazo, proponiendo soluciones fundamentadas', evidence: 'Análisis de impacto completo', feedback: 'Tu análisis de impacto es profundo y propositivo.' }, adecuado: { text: 'Menciona al menos 2 impactos de una decisión', evidence: 'Identificación de impactos', feedback: 'Bien analizas los impactos. Profundiza más.' }, en_desarrollo: { text: 'Reconoce impactos con apoyo', evidence: 'Análisis con guía', feedback: 'Estás aprendiendo a analizar impactos.' }, inicial: { text: 'Necesita apoyo para identificar impactos', evidence: 'Análisis con asistencia', feedback: 'Vamos a pensar en las consecuencias juntos.' } } },
    { name: 'Decisión informada', description: 'Toma decisiones considerando evidencia y perspectivas diversas', descriptors: { avanzado: { text: 'Toma decisiones considerando evidencia, ética y múltiples perspectivas', evidence: 'Decisión fundamentada y ética', feedback: 'Tus decisiones son informadas y responsables.' }, adecuado: { text: 'Toma una decisión considerando al menos 2 factores', evidence: 'Decisión con factores', feedback: 'Bien decides. Considera más factores.' }, en_desarrollo: { text: 'Toma decisiones con guía del docente', evidence: 'Decisión con guía', feedback: 'Estás aprendiendo a tomar decisiones informadas.' }, inicial: { text: 'Necesita apoyo para tomar decisiones', evidence: 'Decisión con asistencia', feedback: 'Vamos a decidir juntos considerando todo.' } } },
  ];
}

function buildParvulariaCriteria(_keywords: string[], _oaText: string, _mainConcept: string): CriteriaTemplate[] {
  return [
    { name: 'Exploración y juego', description: 'Explora el entorno y participa en juegos con curiosidad', descriptors: { avanzado: { text: 'Explora con curiosidad, hace preguntas y participa activamente en juegos variados', evidence: 'Participación activa y curiosa', feedback: 'Tu exploración es muy curiosa y activa.' }, adecuado: { text: 'Participa en juegos y exploraciones con interés', evidence: 'Participación con interés', feedback: 'Bien exploras. Sigue haciendo preguntas.' }, en_desarrollo: { text: 'Participa con apoyo del docente o compañeros', evidence: 'Participación con guía', feedback: 'Estás explorando cada vez más.' }, inicial: { text: 'Necesita modelado para participar', evidence: 'Participación con asistencia', feedback: 'Vamos a explorar juntos.' } } },
    { name: 'Expresión libre', description: 'Se expresa a través del dibujo, la voz, el cuerpo y la creatividad', descriptors: { avanzado: { text: 'Se expresa con variedad de medios y comunica sus ideas con claridad', evidence: 'Expresión variada y comunicativa', feedback: 'Tu expresión es muy creativa y clara.' }, adecuado: { text: 'Se expresa por al menos 2 medios diferentes', evidence: 'Expresión con variedad', feedback: 'Bien te expresas. Prueba nuevos medios.' }, en_desarrollo: { text: 'Se expresa con apoyo y estímulo', evidence: 'Expresión con guía', feedback: 'Estás encontrando tu forma de expresarte.' }, inicial: { text: 'Necesita estímulo para expresarse', evidence: 'Expresión con asistencia', feedback: 'Vamos a expresar juntos lo que sientes.' } } },
    { name: 'Participación grupal', description: 'Participa en actividades grupales respetando turnos y normas simples', descriptors: { avanzado: { text: 'Participa respetando turnos, ayuda a compañeros y disfruta del juego grupal', evidence: 'Participación colaborativa', feedback: 'Eres muy buen compañero/a de juego.' }, adecuado: { text: 'Participa y respeta turnos básicos', evidence: 'Participación con turnos', feedback: 'Bien participas. Sigue respetando a los demás.' }, en_desarrollo: { text: 'Participa con recordatorio de normas', evidence: 'Participación con recordatorio', feedback: 'Estás aprendiendo a jugar con otros.' }, inicial: { text: 'Necesita apoyo constante para participar', evidence: 'Participación con apoyo', feedback: 'Vamos a participar juntos en el juego.' } } },
    { name: 'Relación con otros', description: 'Se relaciona con compañeros y adultos de forma respetuosa', descriptors: { avanzado: { text: 'Se relaciona con empatía, resuelve conflictos con palabras y muestra afecto', evidence: 'Relación empática y respetuosa', feedback: 'Tu forma de relacionarte es ejemplar.' }, adecuado: { text: 'Se relaciona y comparte con al menos 2 compañeros', evidence: 'Relación básica', feedback: 'Bien te relacionas. Sigue compartiendo.' }, en_desarrollo: { text: 'Se relaciona con apoyo del adulto', evidence: 'Relación con guía', feedback: 'Estás aprendiendo a relacionarte mejor.' }, inicial: { text: 'Necesita mediación para relacionarse', evidence: 'Relación con asistencia', feedback: 'Vamos a jugar juntos con los demás.' } } },
  ];
}
