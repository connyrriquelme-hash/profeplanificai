...
export async function onRequestGet(context: EventContext<Env>): Promise<Response> {
  try {
    const [total, conIndicadores, sinIndicadores, porCurso] = await Promise.all([
      ...
      context.env.DB.prepare("SELECT COUNT(DISTINCT o.id) AS total FROM objectives o JOIN objective_indicator_texts oi ON oi.objective_id=o.id WHERE o.type='OA'").first<any>(),
      context.env.DB.prepare("SELECT COUNT(*) AS total FROM objectives o WHERE o.type='OA' AND o.id NOT IN (SELECT DISTINCT objective_id FROM objective_indicator_texts)").first<any>(),
      context.env.DB.prepare(`
        ...
        FROM (
          ...
          FROM objectives o LEFT JOIN objective_indicator_texts oi ON oi.objective_id=o.id
