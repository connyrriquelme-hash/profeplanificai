...
export async function onRequestPost(context: EventContext<Env, string, { provider: string }>): Promise<Response> {
  try {
    ...
    if (provider === 'workers-ai') {
      if (!context.env.AI) return Response.json({ error: 'Workers AI no configurado en el servidor' }, { status: 503 });
      const result = await context.env.AI.run('@cf/meta/llama-3.3-70b-instruct-fp8-fast', {
        messages: [
          { role: 'system', content: systemPrompt || 'Eres especialista en educación chilena.' },
