  ...
  return (
    <div className="theme-calida min-h-screen bg-[var(--bg)]">
      ...
      <div className="lg:hidden flex flex-col items-center pt-8 pb-4 px-4">
        ...
        <p className="text-sm text-[var(--ink-soft)] italic mt-0.5">{SLOGAN}</p>
      </div>
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="hidden lg:flex flex-col justify-center">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-base text-[var(--ink-soft)] italic">{SLOGAN}</p>
              </div>
            </div>

            <p className="text-lg text-[var(--ink)] leading-relaxed mb-8 max-w-lg">
            ...
            <div className="space-y-3">
              {BENEFITS.map((b, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + i * 0.08 }} className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-[var(--ink)]">{b.text}</span>
                </motion.div>
              ))}
            </div>
          ...
          <div className="space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="bg-white rounded-2xl shadow-xl p-8">
              <div className="flex items-center gap-3 mb-1">
                ...
                <h2 className="text-xl font-bold text-[var(--ink)]">Ingreso a ProfePlanificAI</h2>
              </div>
              <p className="text-sm text-[var(--ink-soft)] mb-6 ml-[52px]">Solo pueden acceder administradores y usuarios creados previamente por el administrador.</p>

              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label htmlFor="login-email" className="block text-sm font-medium text-[var(--ink)] mb-1">Email</label>
                  <input id="login-email" name="email" type="email" autoComplete="username" value={email} onChange={e => setEmail(e.target.value)} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--primary)] focus:border-[var(--primary)] outline-none transition" placeholder="docente@colegio.cl" />
                </div>
                <div>
                  <label htmlFor="login-password" className="block text-sm font-medium text-[var(--ink)] mb-1">Contraseña</label>
                  <input id="login-password" name="password" type="password" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--primary)] focus:border-[var(--primary)] outline-none transition" placeholder="mínimo 6 caracteres" />
                </div>
