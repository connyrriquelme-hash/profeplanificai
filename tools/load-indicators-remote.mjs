...
function fetchRemoteObjectiveMapping(codes) {
  const map = new Map();
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const parsed = runCommand(sql, { local: false });
    for (const row of parsed[0]?.results ?? []) map.set(row.code, row);
    console.log(`  mapping objective/course/subject (REMOTO): lote ${i + 1}/${chunks.length}`);
  }
  return map;
}
...
function main() {
  ...
  console.log('Consultando mapping objective/course/subject en D1 REMOTO...');
  const mapping = fetchRemoteObjectiveMapping(toImport.map((r) => r.code));
  ...
  console.log(`\nFilas a insertar en REMOTO: ${rows.length}`);
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    runFile(sql, { local: false });
    inserted += batches[i].length;
    console.log(`  lote ${i + 1}/${batches.length} insertado (${inserted}/${rows.length} acumulado)`);
  }

  console.log(`\nCarga a D1 REMOTO (PRODUCCION) completa: ${inserted} indicadores insertados en ${toImport.length} OA.`);
}
