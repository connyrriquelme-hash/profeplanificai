...
export function Sidebar({ activeView, onViewChange }: SidebarProps) {
        ...
        return (
          <button
            ...
            className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-2xl text-sm font-medium transition-all duration-200 group ${
              isActive
                ? 'bg-indigo-50/80 text-indigo-700 font-semibold shadow-sm border-l-[3px] border-indigo-500'
          ...
          >
            ...
            {isActive && <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 ml-auto animate-scale-in" />}
          </button>
        );
      })}
    </nav>
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-4 py-5 border-b border-gray-100/80">
        ...
        <div>
          ...
          <p className="text-[11px] text-gray-400 font-medium tracking-wide">Suite docente IA</p>
        </div>
      </div>
      {navItems}
      <div className="px-4 py-3 border-t border-gray-100/80">
        <p className="text-center text-[11px] text-gray-400 font-medium tracking-wide">ProfePlanificAI v2.0</p>
      </div>
    </div>
  );
  ...
  return (
    <>
      <button
        className="lg:hidden fixed top-3 left-3 z-50 p-2.5 rounded-2xl bg-white border border-gray-200/80 shadow-md text-gray-600 hover:bg-gray-50 transition-all"
      ...
      <nav
        className={`lg:hidden fixed inset-y-0 left-0 z-40 w-64 min-w-[260px] bg-white/95 backdrop-blur-md border-r border-gray-200/60 flex flex-col transition-transform duration-250 ease-out no-print ${
