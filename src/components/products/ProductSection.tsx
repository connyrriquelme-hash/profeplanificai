  ...
  return (
    <section className={`product-section overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-200/70 break-inside-avoid print:shadow-none ${className || ''}`} style={style}>
      <button
        ...
        className="w-full flex items-center gap-3 px-4 py-3 bg-[var(--primary-tint)] border-b border-slate-100 hover:brightness-95 transition-colors text-left print:bg-white"
        aria-expanded={isExpanded}
        aria-controls={`section-${title.toLowerCase().replace(/\s+/g, '-')}`}
      >
        {icon && <span className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white text-lg shadow-sm ring-1 ring-[var(--border)]" aria-hidden="true">{icon}</span>}
