...
export function ProjectRenderer({ product, className, style, onProductChange }: ProjectRendererProps) {
        ...
        <ProductSection title="Roles del Equipo" icon="👥">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {roles.map((role, index) => (
              <div key={index} className="bg-gray-50 border border-[var(--border)] rounded-lg p-3">
                <h4 className="text-sm font-semibold text-[var(--ink)]">{role.name}</h4>
                <p className="text-xs text-[var(--ink-mid)] mt-1">{role.description}</p>
              </div>
            ))}
          </div>
        </ProductSection>
              ...
              <div key={index} className="relative pl-8">
                ...
                <div className="bg-white border border-[var(--border)] rounded-lg p-3">
                  <div className="flex items-center justify-between mb-1">
                    <h4 className="text-sm font-semibold text-[var(--ink)]">{phase.name}</h4>
                    <span className="text-xs text-[var(--ink-soft)]">{phase.duration}</span>
                  </div>
                  <ul className="space-y-1">
                    {phase.tasks.map((task, ti) => (
                      <li key={ti} className="text-xs text-[var(--ink-mid)] flex items-start gap-1">
            ...
            <table className="w-full border-collapse" role="grid" aria-label="Tareas del proyecto">
              <thead>
                <tr className="bg-gray-50 border-b border-[var(--border)]">
                  <th scope="col" className="px-4 py-3 text-left text-xs font-semibold text-[var(--ink-mid)] uppercase">Tarea</th>
                  <th scope="col" className="px-4 py-3 text-left text-xs font-semibold text-[var(--ink-mid)] uppercase">Responsable</th>
                  <th scope="col" className="px-4 py-3 text-left text-xs font-semibold text-[var(--ink-mid)] uppercase">Plazo</th>
                  <th scope="col" className="px-4 py-3 text-left text-xs font-semibold text-[var(--ink-mid)] uppercase">Estado</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {tasks.map((task, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm text-[var(--ink)]">{task.task}</td>
                    <td className="px-4 py-3 text-sm text-[var(--ink-mid)]">{task.responsible || '—'}</td>
                    <td className="px-4 py-3 text-sm text-[var(--ink-mid)]">{task.deadline || '—'}</td>
                    <td className="px-4 py-3">
                      <span className="inline-flex px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 text-[var(--ink)]">
        ...
        <ProductSection title="Productos Esperados" icon="📦">
          <ul className="space-y-2">
            {products.map((product, index) => (
              <li key={index} className="flex items-start gap-2 text-sm text-[var(--ink)]">
        ...
        <ProductSection title="Criterios de Evaluación" icon="✓">
          <ul className="space-y-2">
            {evaluationCriteria.map((criterion, index) => (
              <li key={index} className="flex items-start gap-2 text-sm text-[var(--ink)] bg-gray-50 p-2 rounded">
