...
function renderMarkdownLine(line: string): React.ReactNode {
  ...
  const html = sanitized
    .replace(/\*\*(.+?)\*\*/g, '<strong style="color:#000000" class="font-semibold">$1</strong>')
    .replace(/\*(.+?)\*/g, '<em style="color:#000000" class="italic opacity-80">$1</em>');
        ...
        <tbody>
          {rows.map((row, ri) => (
            <tr key={ri} className="border-b border-theme-gray/20 last:border-0 hover:bg-theme-beige/50 transition-colors">
              {row.map((cell, ci) => (
                <td key={ci} className="px-4 py-3 whitespace-nowrap" style={{color:'#000000'}}>
      ...
      {slideLesson ? (
        ...
        <Card variant="elevated" className="p-4 sm:p-6 lg:p-8 xl:p-10">
          <div className="bg-white rounded-xl border border-theme-gray/30 shadow-sm p-6 sm:p-8 lg:p-10 xl:p-12">
            <div style={{color:'#000000'}} className="max-w-none leading-relaxed space-y-1">
              {(() => {
                ...
                while (i < bodyLines.length) {
                  ...
                  if (line.startsWith('| ')) {
                    ...
                    if (line.startsWith('# ')) {
                      elements.push(<h1 key={i} className="text-2xl sm:text-3xl font-bold mb-6 pb-4 border-b border-theme-gray/30 tracking-tight" style={{color:'#000000'}}>{line.replace(/^# /, '')}</h1>);
                    } else if (line.startsWith('## ')) {
                      elements.push(<h2 key={i} className="text-xl font-bold mt-8 mb-4 pb-2 border-b border-theme-gray/30 tracking-tight" style={{color:'#000000'}}>{line.replace(/^## /, '')}</h2>);
                    } else if (line.startsWith('### ')) {
                      elements.push(<h3 key={i} className="text-base font-semibold mt-6 mb-3" style={{color:'#000000'}}>{line.replace(/^### /, '')}</h3>);
                    } else if (line.startsWith('- ')) {
                      elements.push(
                        <ul key={i} className="list-disc ml-6 text-sm sm:text-base leading-relaxed mb-1" style={{color:'#000000'}}>
                          <li>{line.replace(/^- /, '')}</li>
                        </ul>
                      );
                      ...
                      elements.push(
                        <ol key={i} className="list-decimal ml-6 text-sm sm:text-base leading-relaxed mb-1" style={{color:'#000000'}}>
                          <li>{content}</li>
                        </ol>
                      );
                      ...
                      elements.push(<p key={i} className="text-sm sm:text-base leading-relaxed mb-2" style={{color:'#000000'}}>{renderMarkdownLine(line)}</p>);
                    }
                    i++;
                  }
                }
