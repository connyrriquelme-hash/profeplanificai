...
// functions/api/materials/rubric.ts sin cambios de comportamiento). Este
// generador es hoy la Capa 2 (fallback) del patrón defensivo de 3 capas:
// se usa cuando no hay AI configurada o cuando la IA no logra validar contra
// RubricaAnaliticaAISchema tras los reintentos de callAIConValidacion().
// ---------------------------------------------------------------------------

export type RubricLevel = { id: string; label: string; score: number; color: string; description: string };
export type RubricIndicator = { levelId: string; descriptor: string; evidence: string; feedbackSuggestion: string };
export type RubricCriterion = { id: string; name: string; description: string; weight: number; indicators: RubricIndicator[] };
...
function buildInclusiveAdjustments(_category: string, isLower: boolean): string[] {
  if (isLower) {
    return [
...
// ---------------------------------------------------------------------------
// RAG curricular + generación por IA + enrichment defensivo (patrón de 3
// capas, mirror de enrichDuaGuide en AIEngine.ts). Fuente de verdad: `DB`
// (planificaia-db) — objectives/curriculum_indicators/skills/attitudes.
// CORE_DB no se usa aquí: solo tiene 10 OA de prueba en producción.
// ---------------------------------------------------------------------------

export interface RubricaContextInput {
  level: string;
  subject: string;
...
export interface RubricaCurricularContext {
  officialObjectiveText: string;
  officialIndicators: string[];
  hasOfficialIndicators: boolean;
  skills: string[];
  attitudes: string[];
}

// Consulta habilidades (OAH) y actitudes (OAA — el proxy más cercano a "OAT"
// que existe en el schema real; no hay tabla OAT explícita) del objetivo.
// El texto oficial del OA y los indicadores ya los consulta el handler
// (rubric.ts) hoy — se reciben aquí para no duplicar esas 2 queries.
export async function fetchRubricaCurricularContext(
  db: D1Database,
  objectiveCode: string,
  officialObjectiveText: string,
  officialIndicators: string[],
): Promise<RubricaCurricularContext> {
  const [skillsRes, attitudesRes] = await Promise.all([
    db.prepare(
      `SELECT sk.official_text FROM objective_skills os
       JOIN skills sk ON os.skill_id = sk.id
       JOIN objectives o ON os.objective_id = o.id
       WHERE o.code = ?`
    ).bind(objectiveCode).all().catch(() => ({ results: [] as unknown[] })),
    db.prepare(
      `SELECT at.official_text FROM objective_attitudes oa
       JOIN attitudes at ON oa.attitude_id = at.id
       JOIN objectives o ON oa.objective_id = o.id
       WHERE o.code = ?`
    ).bind(objectiveCode).all().catch(() => ({ results: [] as unknown[] })),
  ]);

  const skills = ((skillsRes as { results?: Array<{ official_text?: string }> })?.results || [])
    .map((r) => r.official_text)
    .filter((v): v is string => Boolean(v));

  const attitudes = ((attitudesRes as { results?: Array<{ official_text?: string }> })?.results || [])
    .map((r) => r.official_text)
    .filter((v): v is string => Boolean(v));

  return {
    officialObjectiveText,
    officialIndicators,
    hasOfficialIndicators: officialIndicators.length > 0,
    skills,
    attitudes,
  };
}

export function buildCurricularFrameworkBlock(ctx: RubricaCurricularContext, input: RubricaContextInput): string {
  const lines: string[] = ['[MARCO_CURRICULAR_MINEDUC_OFICIAL]'];
  lines.push(`OA (${input.objectiveCode}): ${ctx.officialObjectiveText || input.objectiveText}`);
  lines.push(`Tema de la clase: ${input.topic}`);
  lines.push(`Nivel: ${input.level} — Asignatura: ${input.subject}`);

  if (ctx.hasOfficialIndicators) {
    lines.push('');
    lines.push('INDICADORES DE EVALUACIÓN OFICIALES MINEDUC para este OA (fuente curricular verificada):');
    ctx.officialIndicators.forEach((i) => lines.push(`- ${i}`));
    lines.push('REGLA: los criterios y descriptores de la rúbrica DEBEN derivarse y alinearse estrictamente con estos indicadores oficiales. No los ignores ni los contradigas.');
  } else {
    lines.push('');
    lines.push('No existen indicadores de evaluación oficiales MINEDUC catalogados para este OA específico todavía (cobertura parcial del banco curricular).');
    lines.push('REGLA: deriva los criterios directamente del texto oficial del OA de arriba y de las habilidades/actitudes listadas abajo. NO inventes que citas un indicador oficial que no existe. NO uses esto como excusa para generar criterios genéricos: sigue exigiendo especificidad al tema y nivel.');
  }

  if (ctx.skills.length) {
    lines.push('');
    lines.push('Habilidades curriculares asociadas (OAH):');
    ctx.skills.forEach((s) => lines.push(`- ${s}`));
  }
  if (ctx.attitudes.length) {
    lines.push('');
    lines.push('Actitudes/objetivos actitudinales asociados (OAA, proxy de OAT):');
    ctx.attitudes.forEach((a) => lines.push(`- ${a}`));
  }

  if (input.clientIndicators.length) {
    lines.push('');
    lines.push('Indicadores adicionales sugeridos por el docente (complementarios, no oficiales):');
    input.clientIndicators.forEach((i) => lines.push(`- ${i}`));
  }
...
// aquí a propósito en vez de importarlo desde src/ (Vite): cruzar
// functions/ (Workers) -> src/ (Vite) rompería el bundling de Functions.
...
function isCriterionUsable(criterion: RubricCriterionAI): boolean {
  if (!criterion.name || criterion.name.trim().length < 5) return false;
  if (criterion.indicators.length !== 4) return false;
  const levelIds = new Set(criterion.indicators.map((i) => i.levelId));
  if (levelIds.size !== 4) return false;
  return criterion.indicators.every((ind, idx) => {
    const siblingTexts = criterion.indicators.filter((_, i) => i !== idx).map((i) => i.descriptor);
    return !isGenericOrWeak(ind.descriptor, siblingTexts);
  });
}

// Capa 3 del patrón defensivo: si la IA sí validó contra Zod, fusiona campo a
// campo contra el fallback determinista en vez de aceptarla o descartarla en
// bloque — un criterio débil puntual no tira toda la respuesta de la IA.
function enrichRubrica(ai: RubricaAnaliticaAI, fallback: PremiumRubric, isLower: boolean): PremiumRubric {
  const usableCriteria = ai.criteria.filter(isCriterionUsable);
  const finalCriteria: RubricCriterion[] = usableCriteria.length >= 2 ? usableCriteria : fallback.criteria;
  const totalScore = finalCriteria.length * 4;
  ...
  return {
    ...
    totalScore,
    scoringFormula: isLower
      ? `Puntaje total: ${totalScore} puntos. Cada criterio vale 4 puntos.`
      : `Puntaje total: ${totalScore} puntos. Nota final: (puntaje obtenido / ${totalScore}) × 7.`,
    usageInstructions: ai.usageInstructions && ai.usageInstructions.length >= 3 ? ai.usageInstructions : fallback.usageInstructions,
    inclusiveAdjustments: ai.inclusiveAdjustments && ai.inclusiveAdjustments.length >= 3 ? ai.inclusiveAdjustments : fallback.inclusiveAdjustments,
    studentSelfAssessment: ai.studentSelfAssessment?.prompts?.length >= 3 ? ai.studentSelfAssessment : fallback.studentSelfAssessment,
    // levels/title/subtitle/nivel/asignatura/oa/tema/formativeFeedbackQuestions: siempre del fallback (deterministas, la IA no los genera — ver rubricaAnalitica.ts)
  };
}

// Capa 1 (IA) -> Capa 2 (fallback) -> Capa 3 (enrichRubrica). Punto de
// entrada único llamado desde functions/api/materials/rubric.ts.
export async function generateRubricaContent(
  env: AIEngineEnv,
  input: RubricaContextInput,
  curricularCtx: RubricaCurricularContext,
): Promise<PremiumRubric> {
  const fallback = buildPremiumRubric({
    level: input.level,
    subject: input.subject,
    objectiveCode: input.objectiveCode,
    objectiveText: input.objectiveText,
    topic: input.topic,
    indicators: input.clientIndicators,
    skills: input.clientSkills,
  });

  try {
    const { data } = await callAIConValidacion(
      env,
      buildSystemPromptRubrica(input.topic, input.level),
      buildCurricularFrameworkBlock(curricularCtx, input),
      RubricaAnaliticaAISchema,
      // El modelo default (3B) falla sistemáticamente el schema rico de
      // rúbrica (confirmado contra el servidor real); el 70B ya usado por
      // el orchestrator de Mis Clases lo cumple al primer intento.
