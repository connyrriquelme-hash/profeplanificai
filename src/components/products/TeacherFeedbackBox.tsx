...
export function TeacherFeedbackBox({ feedback = '', className, style, onChange }: TeacherFeedbackBoxProps) {
  ...
  return (
    <div className={`teacher-feedback-box ${className || ''}`} style={style}>
      <label htmlFor="teacher-feedback" className="block text-sm font-medium text-[var(--ink)] mb-2">
      ...
      <textarea
        ...
        rows={4}
        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--primary)] focus:border-[var(--primary)] text-sm text-[var(--ink)] placeholder-gray-400 resize-y min-h-[80px]"
        placeholder="Escribe tu retroalimentación para el estudiante..."
        aria-describedby="teacher-feedback-help"
      />
      <p id="teacher-feedback-help" className="mt-1 text-xs text-[var(--ink-soft)]">
