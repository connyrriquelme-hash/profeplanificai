...
interface SlideLessonPreviewProps {
  lesson: SlideLesson;
  onDeckChange?: (deck: VisualLessonDeck) => void;
  onExportPPTX?: (deck: VisualLessonDeck) => void;
  onExportPDF?: () => void;
  onSave?: () => void;
  onShare?: () => void;
}
...
export function SlideLessonPreview({ lesson, onDeckChange, onExportPPTX, onExportPDF, onSave, onShare }: SlideLessonPreviewProps) {
  ...
  useEffect(() => {
    const normalized = normalizeLessonSlidesToVisualDeck(lesson, { previousDeck: deckRef.current ?? undefined });
    setDeck(normalized);
    onDeckChange?.(normalized);
    // Si todos los slides heredaron su imagen del deck anterior (nada
    // visual cambio), no hace falta disparar otra ronda de generacion.
    autoGeneratedKeyRef.current = normalized.slides.every(s => s.visual.status === 'generated') ? normalized.id : '';
    setCurrentSlide(0);
    setShowNotes(false);
  }, [lesson, onDeckChange]);
  ...
  }, [generatingAll, onDeckChange, showToast]);

  useEffect(() => {
    if (!deck || generatingAll || autoGeneratedKeyRef.current === deck.id) return;
    autoGeneratedKeyRef.current = deck.id;
    void handleGenerateAllImages();
  }, [deck, generatingAll, handleGenerateAllImages]);
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {onExportPPTX && <Button variant="secondary" size="sm" onClick={() => onExportPPTX(deck)}>Exportar PPTX</Button>}
