...
export function CreativeHub({ onSelect }: CreativeHubProps) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {ACTIONS.map(({ tipo, icon: Icon, title, desc, color }) => (
          <div
            ...
            className="rounded-2xl border border-[var(--line)] bg-white p-6 shadow-md hover:scale-105 hover:shadow-lg transition-all duration-200 cursor-pointer"
          >
            <div
      ...
      {isBannerVisible && (
        <div className="relative rounded-2xl bg-orange-50 border border-orange-200 p-6 mb-8 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
          <button
