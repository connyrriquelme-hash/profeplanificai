...
export function CurriculumCallout({
  ...
  return (
    <div
      className={`curriculum-callout bg-[var(--primary-tint)] border border-[var(--primary)]/25 rounded-xl p-4 md:p-5 ${className || ''}`}
      style={style}
      role="complementary"
    ...
    >
      ...
      <div className="space-y-2">
            ...
            <ul className="mt-1 space-y-0.5">
              {skills.map((skill, index) => (
                <li key={index} className="text-sm text-[var(--primary-ink)] flex items-start gap-1.5">
                  <span className="text-[var(--primary)]/60 mt-1 flex-shrink-0">•</span>
                  {skill}
                </li>
              ))}
            </ul>
        ...
        {learningGoal && (
          <div className="mt-2 pt-2 border-t border-[var(--primary)]/25">
