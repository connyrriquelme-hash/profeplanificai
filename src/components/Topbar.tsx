...
export function Topbar({ title, onNavigate }: TopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-20 glass border-b border-theme-gray/30 px-4 lg:px-6 xl:px-8 py-3 flex items-center gap-4 bg-theme-beige/80 backdrop-blur-md">
      {title && (
        <h1 className="text-lg font-semibold text-theme-text lg:hidden">{title}</h1>
      )}
      ...
      <div className="flex items-center gap-2 ml-auto">
        ...
        <button className="p-2.5 rounded-2xl text-theme-secondary hover:text-theme-text hover:bg-theme-gray/20 transition-all" aria-label="Ayuda">
          <HelpCircle size={18} strokeWidth={2.25} />
        </button>
        <button className="p-2.5 rounded-2xl text-theme-secondary hover:text-theme-text hover:bg-theme-gray/20 transition-all relative" aria-label="Notificaciones">
          <Bell size={18} strokeWidth={2.25} />
          <span className="absolute top-2.5 right-2.5 w-2 h-2 rounded-full bg-theme-accent ring-2 ring-theme-beige" />
        </button>
        <div className="flex items-center gap-2.5 pl-3 ml-2 border-l border-theme-gray/30">
