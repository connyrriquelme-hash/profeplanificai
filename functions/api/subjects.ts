...
export async function onRequestGet(context: EventContext<Env>): Promise<Response> {
  ...
  const query = course
    ? context.env.DB.prepare(`SELECT s.*,COUNT(o.id) AS objective_count FROM subjects s JOIN objectives o ON o.subject_id=s.id JOIN courses c ON c.id=o.course_id WHERE c.code=? GROUP BY s.id ORDER BY s.name`).bind(course)
