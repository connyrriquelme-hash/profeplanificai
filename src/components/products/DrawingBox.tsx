...
export function DrawingBox({ 
  ...
  return (
    <div className={`drawing-box ${className || ''}`} style={style}>
      {(title || instruction) && (
        <div className="mb-2">
          {title && <h4 className="font-semibold text-[var(--ink)] mb-1">{title}</h4>}
          {instruction && (
            <p className="text-sm text-[var(--ink-mid)] mb-2">{instruction}</p>
          )}
        </div>
      )}
      <div
      ...
      >
        <div className="w-full h-full flex items-center justify-center">
          <div className="text-center text-gray-400 p-4">
            ...
            <p className="text-[var(--ink-soft)] text-sm">Espacio para dibujo</p>
